...
## 🌐 How to Host This Examination Portal Online (Free & Permanent)

Once your Supabase database is connected, you can host `index.html` online in minutes so candidates can access it from any phone, tablet, or PC!

### 💡 Pro Tip: Pre-Configure Supabase for Examinees
To prevent examinees from having to open "Cloud Settings" and paste your Supabase keys:
...
2. Search for:
   ...
   ```
3. Paste your **Project URL** and public **`anon` key** between the quotes:
   ```javascript
   const DEFAULT_SUPABASE_URL = "https://xyz.supabase.co";
   const DEFAULT_SUPABASE_ANON_KEY = "eyJhbGciOi...";
   ```
4. Save the file. Anyone who visits your hosted link will now connect to your Supabase project automatically!

---

### Method 1: Netlify Drop (Fastest — 30 Seconds, No Account / No CLI Needed)
1. Go to [https://app.netlify.com/drop](https://app.netlify.com/drop) in your web browser.
2. Drag and drop the folder containing your `index.html` file into the upload box.
3. Netlify will immediately deploy your site and provide a free, secure HTTPS link (e.g. `https://cblr-exam-portal.netlify.app`).
4. You can customize the site domain name under **Site settings -> Site details -> Change site name**.

---

### Method 2: GitHub Pages (Permanent, Free & Highly Reliable)
1. Log in to [GitHub](https://github.com) and click **New repository** (e.g. `cblr-rule6-exam`).
2. Upload `index.html`.
3. In your repository, click **Settings** -> **Pages** (on the left menu).
...
5. Within 1 minute, your site will be live at:
   `https://<your-github-username>.github.io/cblr-rule6-exam/`

---

### Method 3: Supabase Storage Public Bucket
If you want everything hosted directly within Supabase:
1. In your [Supabase Dashboard](https://supabase.com/dashboard), click **Storage** on the left menu.
2. Click **New bucket**, name it `cblr-portal`, and toggle **Public bucket** to **ON**. Click **Save**.
3. Open the bucket, click **Upload file**, and choose `index.html`.
4. Click the three dots (`⋮`) next to `index.html` and select **Copy URL**.
5. This URL is a direct public link that loads your portal in any web browser!
